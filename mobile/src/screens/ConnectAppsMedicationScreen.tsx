...
type Props = {
  navigation: NativeStackNavigationProp<OnboardingStackParamList, "ConnectAppsMedication">;
};

export default function ConnectAppsMedicationScreen({ navigation }: Props) {
  const { colors, primary } = useTheme();

  // UI state from useUIStore
  const connectedApps = useUIStore((s) => s.connectedApps);
  const medicationApps = connectedApps.filter((app) => app.category === "medication" && app.isInstalled);
  ...
  const handleAutoDetect = () => {
    navigation.navigate("ConnectAppsAutoDetect", { category: "medication" });
  };
        ...
        <ScrollView className="flex-1 px-10 py-12">
          ...
          <View className="mt-6 space-y-4">
            <Pressable
              ...
              accessibilityRole="button"
              accessibilityLabel="Auto detect medication apps"
            >
              <Text className="text-xl font-semibold" style={{ color: primary }}>
                Auto detect medication apps
              </Text>
            </Pressable>
