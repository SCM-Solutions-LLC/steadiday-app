...
const DATA_BREACH_URL = "https://www.steadiday.com/data-breach.html";
...
export default function DataBreachResponseScreen() {
  ...
  const handleCopyURL = async () => {
    await Clipboard.setStringAsync(DATA_BREACH_URL);
  };

  const handleOpenInBrowser = () => {
    Linking.openURL(DATA_BREACH_URL);
  };

  return (
    <ScreenErrorBoundary screenName="DataBreachResponse">
              ...
              {loading && (
                <View className="absolute inset-0 z-10 items-center justify-center">
                  ...
                  <Text className="text-sm mt-3" style={{ color: colors.textSecondary }}>
                    Loading data breach response...
                  </Text>
                </View>
              )}
              <WebView
                ref={webViewRef}
                source={{ uri: DATA_BREACH_URL }}
